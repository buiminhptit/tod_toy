...
def make_curious_prompt(title, context, questions):
    prompt = f"""Curio's Persona: Curio is a person who has a strong desire to learn and understand things better. Whenever he reads something, he likes to ask questions to satisfy his curiosity. 
<START>
...
def generate_prompt(data_point):
    ...
    context = data_point["context"].strip()
    questions = data_point["questions"]
    # merged_questions = " ::: ".join(questions).strip()
    merged_questions = []
    for i, q in enumerate(questions):
        new_q = f"{i+1}. {q}"
        merged_questions.append(new_q)
    merged_questions = "\n".join(merged_questions)
    prompt = make_curious_prompt(title, context, merged_questions)
    return prompt
